...
public class WorldCupFragment extends Fragment implements  SwipeRefreshLayout.OnRefreshListener{
  ...
  public   WorldCupFragment newInstance(String param1, String param2,Context context) {
    ...
    private  void initData(final Context context) {
        ArrayList<String> cacheData= (ArrayList<String>) CacheUtils.readJson(context, WorldCupFragment.this.getClass().getName() + ".json");
        if (cacheData==null||cacheData.size()==0){
            new Thread(){
                ...
                public void run() {
                    ...
                    JSONParseUtil.parseNetDataStand(context, result, WorldCupFragment.this.getClass().getName() + ".json", liveItems, handler, HANDLE_DATA_OK);
                }
            }.start();
        }else {
            JSONParseUtil.parseLocalDataStand(context,WorldCupFragment.this.getClass().getName()+".json",liveItems, handler, HANDLE_DATA_OK);
        }
  }
    View rootView=null;
  @Override
